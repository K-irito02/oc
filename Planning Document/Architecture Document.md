...
本项目是一个基于 React + Spring Boot 的 oc 产品软件发布平台，采用渐进式微服务架构演进策略（单体 → 微服务），支持软件发布、版本管理、自动更新、用户管理、支付捐赠等核心功能，具备风格定义的界面设计和完整的中英文多语言支持。
...
| Key 模式 | 用途 | TTL | 更新策略 |
|---------|------|-----|---------|
| `oc:auth:session:{userId}` | 用户会话（Refresh Token） | 7 天 | 登录时写入，登出时删除 |
| `oc:auth:blacklist:{tokenId}` | Token 黑名单（已注销的 Access Token） | 2 小时 | 登出时写入 |
| `oc:user:info:{userId}` | 用户信息缓存 | 1 小时 | 用户更新时主动清除 |
| `oc:product:detail:{productId}` | 产品详情缓存 | 30 分钟 | 产品更新时主动清除 |
| `oc:product:list:{hash}` | 产品列表缓存（hash = 查询参数哈希） | 15 分钟 | 任何产品变更时清除所有列表缓存 |
| `oc:product:featured` | 推荐产品缓存 | 1 小时 | 定时刷新 |
| `oc:product:hot` | 热门产品排行 | 1 小时 | 定时计算 |
| `oc:version:latest:{productId}:{platform}` | 最新版本信息 | 10 分钟 | 新版本发布时清除 |
| `oc:stats:download:{productId}` | 下载计数缓存（写入缓冲） | 5 分钟 | 定时批量写回数据库 |
| `oc:stats:view:{productId}` | 浏览计数缓存 | 5 分钟 | 定时批量写回数据库 |
| `oc:limit:login:{ip}` | 登录限流计数器 | 15 分钟 | 自动过期 |
| `oc:limit:api:{userId}` | API 限流计数器 | 1 小时 | 滑动窗口 |
...
public void refreshHotProducts() {
    cacheManager.getCache("oc:product:hot").clear();
    productService.getHotProducts();
}
```
...
示例:
  oc-studio_1.2.0_windows_x64.zip
  oc-studio_1.2.0_linux_x64.tar.gz
  oc-studio_1.2.0_macos_arm64.dmg

增量更新包:
  oc-studio_1.0.0_to_1.2.0_windows_x64.zip
```
...
本技术架构文档为 oc 产品发布平台提供了完整的技术方案，涵盖以下核心内容：
...
7. **更新机制**：oc 客户端自动更新 + 灰度发布 + 增量更新 + 断点续传
